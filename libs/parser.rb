...
class Parser
    ...
    def self.parse_m18(url)
        ...
        result[:image_mid] = doc.search('div#J_zoom img').first['src']
        result
    end
    def self.parse_vancl(url)
        result = {}
        doc = Nokogiri::HTML(open(url));
        result[:title] = doc.search('title').first.content
        result[:name] = doc.search('h2#productname').first.content
        result[:price] = doc.search('.cuxiaoPrice span').first.content
        result[:image_mid] = doc.search('img#midimg').first['src']
        result
    end
    def self.parse_taobao(url)
        result = {}
        doc = Nokogiri::HTML(open(url));
        result[:title] = doc.search('title').first.content
        ...
        result[:image_mid] = doc.search('img#J_ImgBooth').first['src']
        result
    end
    def self.parse_360buy(url)
        result = {}
        doc = Nokogiri::HTML(open(url));
        result[:title] = doc.search('title').first.content
        result
    end
    def self.parse_amazon(url)
        result = {}
        doc = Nokogiri::HTML(open(url));
        result[:title] = doc.search('title').first.content
        result[:name] = doc.search('h1.parseasinTitle').first.content
        result[:price] = doc.search('b.priceLarge').first.content
        result[:image_mid] = doc.search('img#prodImage').first['src']
        result
    end
end
